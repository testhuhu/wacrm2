  ...
  const send = async () => {
    ...
    try {
      const res = await fetch('/api/ai/playground', {
        ...
        body: JSON.stringify({
          messages: next.map((tr) => ({ role: tr.role, content: tr.content })),
        }),
      });
      ...
      <div ref={scrollRef} className="flex-1 space-y-4 overflow-y-auto p-4">
        ...
        {turns.map((tr, i) => (
          <div
            ...
            className={cn(
              'flex gap-2',
              tr.role === 'user' ? 'justify-end' : 'justify-start',
            )}
          >
            {tr.role === 'assistant' && (
            ...
            >
              {tr.content && <p className="whitespace-pre-wrap">{tr.content}</p>}
              {tr.role === 'assistant' && tr.handoff && (
                <p
                  className={cn(
                    'flex items-center gap-1 text-xs text-amber-500',
                    tr.content && 'mt-1.5 border-t border-border/50 pt-1.5',
                  )}
                >
