...
export function WhatsAppConfig() {
  ...
  const tToast = useTranslations('Settings.whatsappToasts');
  ...
  async function handleSave() {
    if (!phoneNumberId.trim()) {
      toast.error(tToast('phoneIdRequired'));
      return;
    }
    if (!config && (!accessToken.trim() || !tokenEdited)) {
      toast.error(tToast('tokenRequired'));
      return;
    }
    ...
    try {
      ...
      if (tokenEdited && accessToken !== MASKED_TOKEN && accessToken.trim()) {
        ...
        toast.error(tToast('tokenReenter'));
        setSaving(false);
        return;
      }
  ...
  async function handleTestConnection() {
    try {
      ...
      setConnectionStatus('disconnected');
      toast.error(tToast('connTestFailed'));
    } finally {
      setTesting(false);
    }
  }
  ...
  async function handleVerifyRegistration() {
    ...
    try {
      ...
      if (data.live) {
        toast.success(tToast('numberWired'));
      } else {
        toast.error(
      ...
      toast.error(tToast('verificationUnreachable'));
    } finally {
  ...
  async function handleReset() {
    ...
    try {
      ...
      toast.success(tToast('configCleared'));
      setConfig(null);
      setPhoneNumberId('');
      setWabaId('');
      setAccessToken('');
